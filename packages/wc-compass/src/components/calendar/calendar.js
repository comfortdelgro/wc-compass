...
export class CdgCalendar extends HTMLElement {
  ...
  attributeChangedCallback(attr, oldValue, newValue) {
    ...
    switch (attr) {
      case 'start-date':
        ...
        this.startDate && this.startDate.setHours(0, 0, 0)
        this.renderSelectedDate()
        break
      case 'end-date':
        this.endDate = newValue ? new Date(newValue) : null
        this.endDate && this.endDate.setHours(0, 0, 0)
        this.renderSelectedDate()
        break
      case 'open':
  ...
  renderCell(params) {
    ...
    if (this.min || this.max) {
      ...
      const isDisable =
        (this.min && params.date.getTime() < minDate.getTime()) ||
        (this.max && params.date.getTime() > maxDate.getTime())
      if (isDisable) {
  ...
  createYear() {
    ...
    for (let index = 0; index < DISPLAY_YEARS_LENGTH; index++) {
      ...
      yearElement.addEventListener('click', this.handleYearItemClick.bind(this))
      // Add content
      this.multiYearView
  ...
  initMonthSelectors() {
    const previousMonthSelector = this.querySelector('.previous-month-selector')
